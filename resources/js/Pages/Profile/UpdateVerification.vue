  ...
  const submitVerification = () => {
    ...
    router.post('/verify-update', { code: verificationCode.value }, {
      onSuccess: () => {
      ...
      onError: (errors: any) => {
        ...
        verificationError.value = errors.message || 'Verification failed';
    ...
    <AuthenticatedLayout>
      <main class="flex flex-1 gap-5 mx-auto container py-5 justify-center font-montserrat">
        <div class="h-[370px] w-[500px] bg-secondary rounded-lg p-6">
          <h2 class="text-white text-2xl font-bold mb-4 text-center">Verification</h2>
          ...
          <div v-if="verificationError" class="text-red-500 text-sm mb-4 text-center">
            {{ verificationError }}
          </div>
        
          <form @submit.prevent="submitVerification" class="space-y-4">
            <div>
              <label for="verification-code" class="text-white block mb-2">
                Verification Code
              </label>
              <input 
